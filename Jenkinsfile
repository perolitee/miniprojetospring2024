        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                withSonarQubeEnv(env.SONARQUBE_SERVER) {
                    script{
                        if (params.useSonar){
                            sh "${env.SONAR_SCANNER_HOME}/bin/sonar-scanner -Dsonar.projectKey=miniprojetospring -Dsonar.host.url=http://192.168.0.7:9000 -Dsonar.login=squ_9cd793fa6e1a29c3c11395d4aa8ba43a0104fb59 -Dsonar.sources=src/main/java/ -Dsonar.java.binaries=target/classes"
                        } else {
